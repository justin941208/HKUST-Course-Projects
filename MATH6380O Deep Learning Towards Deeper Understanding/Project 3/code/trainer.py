...
class Trainer(object):
    ...
    def __init__(self, config, data_loader):
        ...
        self.plot_dir = './plots/' + self.model_name + '/' + self.mode
        '/'
        ...
        with tqdm(total=self.num_train) as pbar:
            for i, (x, y) in enumerate(self.train_loader):
                ...
                for t in range(self.num_glimpses):
                    h_t, l_t, b_t, logit, p = self.model(x, l_t, h_t)
                ...
                # calculate reward
                predicted = torch.max(logits[-1], 1)[1]
                ...
                for i in range(len(logits)):
                    loss_action += 0.1 * i / len(logits) * F.cross_entropy(logits[i], y)
                ...
                if plot:
                    if self.use_gpu:
                        ...
                        logits = [F.softmax(lg[0:9], dim=1).cpu().data.numpy().squeeze() for lg in logits]
                    else:
                        imgs = [g.data.numpy().squeeze() for g in imgs]
                        locs = [l.data.numpy() for l in locs]
                        logits = [F.softmax(lg[0:9], dim=1).data.numpy().squeeze() for lg in logits]
                    pickle.dump(
                        imgs, open(
                    ...
                    pickle.dump(
                        logits, open(
                            self.plot_dir + "lg_{}.p".format(epoch + 1),
                            "wb"
                        )
                    )
                # log to tensorboard
    ...
    def validate(self, epoch):
        ...
        for i, (x, y) in enumerate(self.valid_loader):
            ...
            log_probas = F.log_softmax(logits[-1], dim=1)
            # average
